...
**Let's look at the financial obligations of Apple and Mercedez [10 min]**

Links for the annual reports of [Apple](https://investor.apple.com/investor-relations/sec-filings/) and [Mercedez](https://group.mercedes-benz.com/investors/reports-news/annual-reports/2023/).

From the Balance Sheet, check total amount on financial liabilities.
...
- Mercedez: Check Note 24 - Financing liabilities.
...
- Capital lease: substantial transference of all the benefits and risks of ownership to the lessee.
  - Recognition of a new asset (_right-of-use asset_) and lease liability.
- Operating lease: all other leases.
  - Recognizing just rental expenses.
 
Until 2019, firms reporting using IFRS have a strong incentive to structure leases as operating leases even when they are economically equivalent to capital leases.

- Operating leases were not reported on the balance sheet.
...
However, lesses that already adopted IFRS 16 (2019) must capitalize all leases.
...
**Example Apple and Mercedez [10 min]**
Apple: Note 8 - Leases.

Mercedez: Note 12 - Equipment in operating leases
...
**Example Apple and Mercedez [15 min]**

Search: Balance sheet "Commitments and Contingencies."

Apple: "Note 12 " and "Item 3: legal proceedings".

Mercedez: "Note 23 Provisions for other risks", "Note 30 Legal proceedings", and "Note 31 – Contingent liabilities ..." 
...
1. The company creates a separate entity and provides financing _not exceeding 50%_ of the total capital.
   - Under **IFRS 11**, joint ventures are typically accounted for using the **equity method** and **not consolidated**, unless the company has control (per **IFRS 10**).
2. The separate entity borrows money from a bank and uses the money to purchase assets.
3. The separate entity rents the assets to the company.

:::{.notes}
Control is determined based on the ability to direct relevant activities and exposure to variable returns.
The focus is on substance over form.
:::
...
- **Under IFRS 10**, control over the SPE is assessed. 
   - If control exists, the SPE must be consolidated, even if ownership is less than 50%.
   - but if SPE is not controlled by the company, then there is no obligtion to consolidate.

How companies could use SPE to hide debt (don't do it!)
...
1. The company, called the _sponsor_, forms a separate entity and capitalizes it with equity investment, some of which must be from independent third parties.
   1.1 legally, the sponsor must not control the SPE.
...
1. Company sells their receivables to SPE, which issues debt to finance the purchase.
   - (-)Receivables and (+) Cash.
2. The SPE issues bonds collateralized by the receivables it holds.
...
1. Companies execute a contract with an SPE to purchase the output of a plant.
