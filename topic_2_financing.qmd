...
a. Paying off the original loan in the 5th year.
b. Refinancing the previous loan but with a 10% interest rate (principal is not paid, debt restructure)
...
- Non-consolidation: When the parent company holds less than 50% of the JV' shares with voting rights (or lacks control per IFRS 10), the JV’s assets and liabilities are not consolidated into the parent company’s financial statements. This keeps debt associated with the JV off the balance sheet of the parent company.
...
1. The company, called the _sponsor_, forms a separate entity and capitalizes it with equity investment, some of which must be from independent third parties.
   -   again, the sponsor does not control the SPE to avoid consolidation.
2. The SPE issues debt and uses the proceeds to purchase earnings assets from or for the sponsoring company (i.e., receivables)
3. The sponsoring company receives the cash from the transaction and removes the earnings assets from its balance sheet, without recognizing a liability.
...
2. Treasury stock (buybacks): The amount of capital stock repurchased by the company and kept in their own treasury. 
    ...
    -  It reduces both contributed capital and total equity (_contra-equity account_).
...
-  Stock options in executive compensation and bonus plans calling for the issuance of capital stock over some time at fixed prices (i.e., employee stock ownership plan)
