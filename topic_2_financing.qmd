...
A contingent liabilityis a potential obligation that depends on the occurrence of a future uncertain event (e.g., legal disputes or warranties).
...
The company believes it is likely (but not certain) that it will lose the lawsuit.
...
A joint venture is a business entity created by two or more parties that pool resources shared ownership.
Shared ownership, risks, and return.
...
JVs are also used strategically for financial structuring, especially to avoid recognizing liabilities on the parent company's balance sheet. In this context:
